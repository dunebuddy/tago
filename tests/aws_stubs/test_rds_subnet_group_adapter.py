"""AWS stub tests for RDS subnet group adapter tagging behavior."""

import boto3
from botocore.stub import Stubber

from core.arn import Arn
from core.adapters.rds_subnetgroup import RDSSubnetGroupTagAdapter
from core.models import TagSet


def test_rds_subnet_group_apply_tags_calls_add_tags_to_resource(monkeypatch):
    session = boto3.session.Session(region_name="us-east-1")
    client = session.client("rds")
    stubber = Stubber(client)

    arn_str = "arn:aws:rds:us-east-1:123456789012:subgrp:my-subnet-group"
    ...
    with stubber:
        adapter = RDSSubnetGroupTagAdapter(arn, session)
        result = adapter.apply_tags(tagset, dry_run=False, override=True)

    assert result.pretty_name == "RDS Subnet Group"
